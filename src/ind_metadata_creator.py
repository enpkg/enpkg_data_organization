...
input_df['clean_filename'] = input_df['filename'].map(lambda x: x.lstrip('20220513_PMA_').rstrip('.mzML'))
...
individual_folders_path = input_df.apply(lambda x: rootdir /
...
input_df['sample_type'] = np.where(input_df['filename'].str.contains('Ctrl'), 'blank', 'sample')


cols_to_keep = ['filename', 'ipen', 'jbuf_name', 'localisation', 'matched_name',
       'fuzzy_score', 'query_otol_kingdom', 'query_otol_phylum',
       'query_otol_class', 'ATTRIBUTE_query_otol_order',
       'ATTRIBUTE_query_otol_family', 'ATTRIBUTE_query_otol_genus',
       'ATTRIBUTE_query_otol_species', 'rank', 'wd_link',
       'ATTRIBUTE_sample_prep_mode', 'clean_filename']




for csvfile, data in input_df.groupby(individual_folders_path):
    try:
        # print(csvfile, data)
        # csvfile.parent.mkdir(parents=True, exist_ok=True)
        filename = data['clean_filename'].values[0] + '_metadata.tsv'
